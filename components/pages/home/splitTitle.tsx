...
const splits: Record<string, Array<{ path: string; center: number[]; }>> = {
  earn: [
    { path: "path('M 0.0,0.0 L 0.0,11.88 L 53.63,4.44 L 35.36,0.0 L 0.0,0.0 z')", center: [20.21, 4.66] },
    { path: "path('M 0.0,11.88 L 0.0,26.28 L 32.37,30.27 L 71.93,8.9 L 53.63,4.44 L 0.0,11.88 z')", center: [31.0,16.78] },
    { path: "path('M 71.93,8.9 L 88.4,0.0 L 85.68,0.0 L 53.63,4.44 L 71.93,8.9 z')", center: [71.54,4.3] },
    { path: "path('M 85.68,0.0 L 35.36,0.0 L 53.63,4.44 L 85.68,0.0 z')", center: [58.22,1.48] },
    { path: "path('M 0.0,26.28 L 0.0,36.0 L 21.76,36.0 L 32.37,30.27 L 0.0,26.28 z')", center: [12.85,31.77] },
    { path: "path('M 21.76,36.0 L 78.88,36.0 L 32.37,30.27 L 21.76,36.0 z')", center: [44.34,34.09] },
    { path: "path('M 78.88,36.0 L 87.04,36.0 L 122.41,21.17 L 71.93,8.9 L 32.37,30.27 L 78.88,36.0 z')", center: [77.35,23.85] },
    { path: "path('M 122.41,21.17 L 136.0,15.48 L 136.0,0.0 L 88.4,0.0 L 71.93,8.9 L 122.41,21.17 z')", center: [109.95,8.67] },
    { path: "path('M 87.04,36.0 L 136.0,36.0 L 136.0,24.48 L 122.41,21.17 L 87.04,36.0 z')", center: [118.05,30.38] },
    { path: "path('M 136.0,24.48 L 136.0,15.48 L 122.41,21.17 L 136.0,24.48 z')", center: [131.47,20.38] },
  ]
};
...
const SplitTitle: FunctionComponent<SplitTitleProps> = ({
  ...
  const [currentSplits, setCurrentSplits] = useState(splits[currentWord]);
  let list = useRef<Array<SVGUseElement | null>>([]);
  let fadeOutTween;
  let fadeInTween;

  function fadeInNewList() {
    fadeOutTween = null;
  ...
  return (
    ...
    <div className={`${styles.container} ${className || ''}`}>
      <svg width="135.264" height="35.328" viewBox="0 0 135.264 35.328" xmlns="http://www.w3.org/2000/svg" overflow="visible">
        <defs>
          <g id="earn" strokeLinecap="round" fillRule="nonzero" fontSize="9pt" strokeWidth="0">
        ...
        {
          currentSplits.map((item, i) => (
              <use clipPath={item.path}   
                id={`path${i}`} 
                key={`path${i}`}
                style={{ "transformOrigin": `${item.center[0]}px ${item.center[1]}px` }}
                href={`#${currentWord}`}
                vectorEffect="non-scaling-stroke" 
                ref={ e => { list.current[i] = e } } />
          ))
        }
      </svg>
    </div>
